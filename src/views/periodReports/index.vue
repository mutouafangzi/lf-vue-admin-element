<template>
  <div class="period-reports-wrap">
      ...
      <el-row class="filter-option">
        ...
        <el-col :span="2" :offset="6" class="search-right pull-right">
          <el-button type="primary" @click="handleClickSearch" >
            <i class="icon iconfont icon-Search"></i>搜索</el-button>
        </el-col>
      </el-row>
    </div>
        ...
        <div slot="body">
          ...
          <div class="handleBtn">
            <el-button type="primary" size="small" :plain="true" @click="addBtn">
              <i class="icon iconfont icon-Plus"></i>新增</el-button>
            <el-button type="danger" size="small" :plain="true" @click='deletebtn'>
              <i class="icon iconfont icon-Remove"></i>删除</el-button>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-button :plain="true" type="text" size="small" class="color-danger"
                             @click="watchDetail(scope.$index, scope.row)"><i class="icon iconfont icon-Eye-open"></i>查看
                  </el-button>
                  <el-button :plain="true" type="text" @click="editDetail(scope.$index, scope.row)" size="small">
    ...
    <!--新增弹窗-->
    <el-dialog title="新建报表任务" :visible.sync="addReportDialog" class="add-report-dialog" width="900px">
      <el-form
        :model="addReportForm"
        ref="addReportForm"
        prop="addReportForm"
        ...
        <el-form-item size="small" label="报表模板" prop="reportTemplete">
          <el-select
            v-model="addReportForm.reportTemplete"
            placeholder="请选择报表模板">
            <el-option :label="item.label" :value="item.value" :key="index" v-for="(item,index) in addReportTempletes"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item prop="reportPeriodType" label="报表周期">
          <div class="task-circle-wrap">
            <div class="task-circle-scope" style="display: inline-block;vertical-align: top;">
              <el-select
                v-model="addReportForm.reportPeriodType"
                placeholder="请选择报表周期">
                <el-option :label="item.label" :value="item.value" :key="index" v-for="(item,index) in addReportPeriods"></el-option>
              </el-select>
            </div>
          </div>
        </el-form-item>
        <el-form-item prop="checkedUserList" label="通知责任人">
          <template>
            <a-transfer
              ...
              @getCheckedData="getAddCheckedData"></a-transfer>
          </template>
        </el-form-item>
        <el-form-item
          label="其他收件人"
    ...
    <el-dialog title="编辑报表任务" :visible.sync="editReportDialog" class="report-dialog" width="1000px">
      <el-form
        ...
        <el-form-item size="small" label="报表模板" prop="reportTempleteId">
          <el-select
            v-model="editReportForm.reportTemplete"
            placeholder="请选择报表模板">
            <el-option :label="item.label" :value="item.value" :key="index" v-for="(item,index) in editReportTempletes"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item size="small" label="报表格式" prop="reportDownType">
        ...
        <el-form-item prop="reportPeriodType" label="报表周期">
          <div class="task-circle-wrap">
            <div class="task-circle-scope" style="display: inline-block;vertical-align: top;">
              <el-select
                v-model="editReportForm.reportPeriodType"
                placeholder="请选择执行周期">
                <el-option :label="item.label" :value="item.value" :key="index" v-for="(item,index) in editReportPeriods"></el-option>
              </el-select>
            </div>
          </div>
        </el-form-item>
        <el-form-item prop="checkedUserList" label="通知责任人">
          ...
          <template>
            <a-transfer
              ...
              @getCheckedData="getEditCheckedData"></a-transfer>
          </template>
        </el-form-item>
        <el-form-item
          label="其他收件人"
    ...
    <!--查看弹窗-->
    <el-dialog title="查看报表任务" :visible.sync="watchReportDialog" class="report-dialog" width="1000px">
      <el-dialog
        width="900px"
        title="效果预览"
        :visible.sync="resultPreview"
        append-to-body>
            ...
            <grid-item
              ...
              <div :style="style(item)" class="echarts-wrap">
                <component
                  ...
                  :chartData="getEchartsData(item.id)"></component>
              </div>
            </grid-item>
          </grid-layout>
        </section>
